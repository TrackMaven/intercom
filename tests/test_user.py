...
def test_extract_companies():
    ...
    companies = {
        "companies": [
            {"company_id": 123, "name": "Jon and Cam's house of mirrors", "type": "company", "price": "3 doll hairs"}
        ]
    }

    output = user._extract_companies(companies)
    assert output == [{"company_id": 123, "name": "Jon and Cam's house of mirrors"}]
...
def test_create_user(mock_request, mock_params):
    ...
    expected = {"name": name, "email": email, "update_last_request_at": False, "new_session": False}
...
def test_update_last_seen(mock_request):
    ...
    mock_request.assert_called_once_with("POST", "users", data={"user_id": user_id, "last_request_at": last_request_at})
...
def test_bulk_create_users(mock_request):
    ...
    mock_request.assert_called_once_with("POST", "users", data={"users": users})
...
def test_get(mock_request):
    ...
    mock_request.assert_called_once_with("GET", "users", params={"email": "youvulcant@yahoo.com"})
...
def test_delete_user(mock_request, mock_params):
    ...
    mock_request.assert_called_once_with("DELETE", "users", params={"email": "yaherd@perd.com"})
...
def test_save(mock_request):
    ...
    mock_request.assert_called_once_with(
        "POST", "users", data={"name": "Bender Rodriguez", "update_last_request_at": False, "new_session": False}
    )
...
def test_delete(mock_request):
    ...
    user.delete()
    mock_request.assert_called_once_with("DELETE", "users", params={"user_id": "123"})
