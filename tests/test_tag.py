...
def test_tag_users(mock_request):
    ...
    mock_request.called_once_with('POST', 'tags', data={'users': [
        {'user_id': 1, 'untag': False}, {'user_id': 2, 'untag': False}]})
...
def test_untag_users(mock_request):
    ...
    mock_request.called_once_with('POST', 'tags', data={'users': [
        {'user_id': 1, 'untag': True}, {'user_id': 2, 'untag': True}]})
    assert tag.name == 'silly'
...
def test_tag_companies(mock_request):
    ...
    mock_request.called_once_with('POST', 'tags', data={'companies': [
        {'id': 1, 'untag': False}, {'id': 2, 'untag': False}]})
    assert tag.name == 'silly'
...
def test_tag_companies(mock_request):
    ...
    tag = Tag.tag_companies('silly', companies)
    mock_request.called_once_with('POST', 'tags', data={'companies': [
        {'id': 1, 'untag': True}, {'id': 2, 'untag': True}]})
    assert tag.name == 'silly'


@mock.patch('intercom.client.IntercomAPI.request')
def test_list(mock_request):
    mock_request.return_value = [{'name': 'red', 'id': 1}, 
                                 {'name': 'blue', 'id': 2}]
    tags = Tag.list()
